    ...
    <AnimatePresence>
      {!done && (
        <motion.div
          ...
          onClick={onSkip}
          style={{ touchAction: 'manipulation' }}  // не блокируем жесты скролла
        >
          {/* animated grain */}
          ...
          <motion.div
            ...
            style={{
              WebkitMaskImage:'radial-gradient(70% 70% at 50% 50%, rgba(0,0,0,1) 50%, rgba(0,0,0,0) 100%)',
              maskImage:'radial-gradient(70% 70% at 50% 50%, rgba(0,0,0,1) 50%, rgba(0,0,0,0) 100%)',
            }}
          />

          <div className="relative h-full w-full flex items-center justify-center p-6">
            <div className="w-[min(92vw,560px)]">
              <motion.div initial={{ y: 8, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="mb-6 text-center select-none">
                <div className="text-2xl font-black tracking-[0.18em] uppercase">{brand || ' '}</div>
              </motion.div>

              {/* progress bar */}
              <div className="h-2 w-full rounded bg-white/10 overflow-hidden">
                <motion.div className="h-full rounded relative" style={{ width: '0%' }} animate={{ width: `${pct}%` }} transition={{ duration: 0.25, ease: 'easeOut' }}>
                  <div className="absolute inset-0 bg-white/80 [background-image:linear-gradient(45deg,rgba(0,0,0,.0)25%,rgba(0,0,0,.12)25%,rgba(0,0,0,.12)50%,rgba(0,0,0,.0)50%,rgba(0,0,0,.0)75%,rgba(0,0,0,.12)75%,rgba(0,0,0,.12)100%)] [background-size:24px_24px] animate-diag" />
                </motion.div>
              </div>
